...
for n = 1:nnt 
    ...
    Xbar0 = Xbar;                    Ybar0 = Ybar; 
    Zx   = X-Xbar;          Zy   = Y-Ybar; 
     tmp1 = (-H_Phi).*(K*(Zx+(eee-1).*(Xbart.*Zx+Ybart.*Zy).*Xbart)+resist.*Xbart); 
...
%     Anisotropic velocity distribution stronger in direction of cell centroid velocity     
    ...
    % calculate the stresss tensor      
      ...
      [Vn,NormalVecX,NormalVecY,nSn,nSn_ave,Ux,Uy,Vx,Vy,tp1,tp2,sigma11,sigma22,sigma12] = ...
          RadialModelTwo_NormalStressV6CORRECTEDoffsetsigma(X,Y,Phi0,Phi,U,V,p,mu,lmd,theta,beta,zz,zz1,eee,offset,lambda,K,gamma,resist,expN,Hev_eps,Delta_eps,Xbart,Ybart);   
