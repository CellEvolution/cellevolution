% This code use the finite difference method (FDM) and level set method to solve
% the regularized delta function model of the cell evolution under stress.
% The elastic model is PLANE-STRAIN
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Use the Radial Model Two!
%  I choose Lx > Ly so that we can observe longtime evolution along x-axis.
%  Therefore, all the codes related to L need to change. 
%  Zhiwen Zhang-----------2017-4-1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%RadialModelTwo_V5CORRECTEDtwice%%%%%%%%%%%%%%%
%% parameter
theta = 0.0;                        % shrinking quantity in stress, rename to aviod the conflict with the "delta---regularized delta function)"
...
sigma  = 0.0;                       % initial constant for plane-strain uniaxial stress
beta   =2.5;                        % control the amplitude of the nonlinear velocity in G(z)
zz   = 1;                           %coeff. in denominator of G(z)
zz1 = 1;                            %also coeff. in denominator of G(z)
eee = 4;                            %ellipticity coefficient in velocity distribution; eee=1 is isotropic circular symmetry
offset = 0;
f0 = 1;  expN = 1;
...
h = 1/16;        
%% %%%%%%%%%%%%% parameters used in  regularization and reinitialization 
...
eps_diff = 2*h*h*h/4;                     % parameter used in the diffusive term when reinitialize level set! ?
%% %%%%%%%%%%%%%% parameters used in setting mesh grid  and time-step %%%%%%%%%%%%%%
filename1  = 'DataFragmentpush_tf20_tss200_h1over16';    % adaptively compute self-stress tensor
filename2 = 'DataFragmentpush_tf20_tss200_h1over16_ext';
figure_id0 = 217;
% snapshot time data 
dt = h/20; t0 = 0.00; tf = 20;  tt = t0:dt:tf-dt;  nnt = length(tt);     % computational time, sigma = +2.0; 
tss=200;  itv=round(nnt/tss);  
% generate the fine mesh grid and related data structure  
Lx = 20;  Ly = 8;  [X,Y] = meshgrid(-Lx:h:Lx,-Ly:h:Ly);       % NEW CODE for rectangular domain
p           = [X(:),Y(:)];     
...
DBCx        = @(x,y) (lmd+2*mu)/4/mu/(lmd+mu)*sigma*x;
DBCy        = @(x,y) -      lmd/4/mu/(lmd+mu)*sigma*y;
...
X0 = -0;  a0 = 0.6; 
b0 = 3.4;
n0 = 2; r0 = 1.6; c0=1; c1=1;
rr1  = ((sqrt((X/c0-X0).^2+((Y-0)/c1).^2)-r0)/a0).^n0;
rr = sqrt((X/c0-X0).^2+(Y/c1).^2);
rr3 = sqrt((X-5/c0-X0).^2+(Y/(2*c1)).^2);
rr2  = (atan2((Y-0)/c1,X/c0-X0)/b0).^n0;
Phi  = -(rr - 2); %annulus
Phi0 = Phi;   % record the original shape 
...
id_j    = 1;
%% compute 
% Calculate the stiffness matrix and load vector. CAUTION: this part is INVARIANT respect to time
...
[Phi] = ReInitialization_LevelSetADDF(Phi,h,dt,eps_sign,eps_diff,500*Ntau);   % ADd DiFfusive term; 
snapID = 0;  
Xbart = 0;         Ybart = 0.0;      
for n = 1:nnt 
    t = tt(n); 
    ...
    Phix(:,1) = (Phi(:,2)-Phi(:,1))/h;   Phix(:,end) = (Phi(:,end)-Phi(:,end-1))/h;    % Modify the boundary terms
    Phiy(1,:) = (Phi(2,:)-Phi(1,:))/h;   Phiy(end,:) = (Phi(end,:)-Phi(end-1,:))/h;  
    H_Phi     = Hev_eps(Phi);            D_Phi     = Delta_eps(Phi);
    ...
    Zx   = X-Xbar;          Zy   = Y-Ybar; 
     tmp1 = (-H_Phi).*(K*(Zx+(eee-1).*(Xbart.*Zx+Ybart.*Zy).*Xbart)+resist.*Xbart) + (-10)*Hev_eps(1-rr3).*Hev_eps(0.8-t); 
     tmp2 = (-H_Phi).*(K*(Zy+(eee-1).*(Xbart.*Zx+Ybart.*Zy).*Ybart)+resist.*Ybart);
%     Anisotropic velocity distribution stronger in direction of cell centroid velocity     
    ...
    % calculate the stresss tensor  
      ...
      [Vn,NormalVecX,NormalVecY,nSn,nSn_ave,Ux,Uy,Vx,Vy,tp1,tp2] = ...
          RadialModelTwo_NormalStressV6CORRECTEDoffset(X,Y,Phi0,Phi,U,V,p,mu,lmd,theta,beta,zz,zz1,eee,offset,lambda,K,gamma,resist,expN,Hev_eps,Delta_eps,Xbart,Ybart);   
    ...
    if mod(n,itv) == 0;
        ...
        fprintf(1,'Nx=%4d,Ny=%4d, Ntau=%2d, sigma=%.1f, finish update time t=%.4f of total tf=%.4f \n',Nx,Ny,Ntau, sigma, tt(n),tf);
        save(filename1,'tt','s','Phisnap','dragXsnap','dragYsnap','X','Y','Lx','Ly','Phi0','xbars','ybars','resist');    
