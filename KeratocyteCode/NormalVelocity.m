...
function [Vn,NormalVecX,NormalVecY,nSn,nSn_ave,Ux,Uy,Vx,Vy,tp1,tp2,sigma11,sigma22,sigma12] = ...
         NormalVelocity(X,Y,Phi0,Phi,U,V,p,mu,lmd,beta,eee,lambda,K,gamma,Hev,Del,Xbart,Ybart)
%% parameter
h         = (p(2,2)-p(1,2));  
...
sigma11 = (lmd+2*mu)*Ux + lmd*Vy;
sigma22 =  lmd*Ux       +(lmd+2*mu)*Vy;
sigma12 =  mu*(Uy+Vx);
%% compute
...
Vm        = -beta.*nSn./(1+abs(nSn)) + lambda.*(1-area_ratio); 

Erx0 = (erx0+(eee-1)*(Xbart.*erx0+Ybart.*ery0).*Xbart);
Ery0 = (ery0+(eee-1)*(Xbart.*erx0+Ybart.*ery0).*Ybart);
%     Polarized velocity distribution stronger in direction of cell centroid velocity     
Vn         =Vm + gamma.*(Erx0.*NormalVecX + Ery0.*NormalVecY);
%     Polarized velocity distribution stronger in direction of cell centroid velocity    
%  The -1 in the last term accounts for lab frame
 tp1 = Vm;
 tp2 = gamma.*(Erx0.*NormalVecX + Ery0.*NormalVecY);
